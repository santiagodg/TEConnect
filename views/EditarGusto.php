<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_REQUEST['action']) && $_REQUEST["action"]=="newGusto") {
        if ($_POST['id_user']!='' && $_POST['id_ambito']!='') {
            $sql = "INSERT INTO DetalleAmbito_Gusto VALUES('".$_POST['gusto']."',".$_SESSION['id'].",".$_POST['id_ambito'].");";
  ...
  <body>
    ...
    <div class="container my-5 border rounded bg-light shadow p-3">
      <div class="row py-5">
        ...
        <h4 class="mb-5">Gustos</h1>
        <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">
            <p>
                <label class="form-check-label" >Descripcion del gusto</label>
                <input type="text" name="gusto" class="form-control"  value="<?php echo $gusto;?>">
            </p>
            ...
            <button type="submit" class="btn btn-primary">Agregar Gusto</button>
    </form><br>
    <?php
        $sql = "SELECT * FROM DetalleAmbito_Gusto WHERE ID_User=".$_SESSION["id"]." AND ID_Ambito=".$_GET["id_ambito"];
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td><a href=\"".$_SERVER['PHP_SELF']."?action=deleteGusto&id_ambito=".$row["ID_Ambito"]."&id_user=".$row["ID_User"]."\">Borrar</a></td>";
                echo "</tr>";
            }
        }
        echo "</table>";

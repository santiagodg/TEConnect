<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_REQUEST['action']) && $_REQUEST["action"]=="newInteres") {
        if ($_POST['id_user']!='' && $_POST['id_ambito']!='') {
            $sql = "INSERT INTO DetalleAmbito_Interes VALUES('".$_POST['interes']."',".$_SESSION['id'].",".$_POST['id_ambito'].");";
...
<html>
  <head>
    ...
    <title>TEConnect | Intereses</title>
  </head>
  <body>
    ...
    <div class="container my-5 border rounded bg-light shadow p-3">
      <div class="row py-5">
        ...
        <h4 class="mb-5">Intereses</h1>
        <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">
            <p>
                <label class="form-check-label" >Descripcion del interes</label>
                <input type="text" name="interes" class="form-control"  value="<?php echo $interes;?>">
            </p>
            ...
            <button type="submit" class="btn btn-primary">Agregar Interes</button>
    </form><br>
    <?php
        $sql = "SELECT * FROM DetalleAmbito_Interes WHERE ID_User=".$_SESSION["id"]." AND ID_Ambito=".$_GET["id_ambito"];
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td><a href=\"".$_SERVER['PHP_SELF']."?action=deleteInteres&id_ambito=".$row["ID_Ambito"]."&id_user=".$row["ID_User"]."\">Borrar</a></td>";
                echo "</tr>";
            }
        }
        echo "</table>";
